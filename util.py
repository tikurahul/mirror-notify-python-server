...
from model import Credentials, Users
...
def load_session_credentials(request_handler):
    ...
    user_email = session.get_secure_cookie(name='user_email')

    if userid:
        return userid, user_email, StorageByKeyName(Credentials, userid, 'credentials').get()
    else:
        return None, None, None


def store_userdetails(request_handler, userid, user_email):
    """Store current user's ID in session."""
    session = sessions.LilCookies(request_handler, SESSION_SECRET)
    session.set_secure_cookie(name='userid', value=userid)
    session.set_secure_cookie(name='user_email', value=user_email)
